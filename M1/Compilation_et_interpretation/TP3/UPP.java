...
class UPPNot extends UPPUnOp {
    ...
    RTLInst toRTL (ArrayList<Pair<String,PRegister>> locals,
                   ArrayList<String> globals, PRegister reg, RTLInst succ) {
        return new RTLXOri(e.getPRegister(locals),reg,succ);
    }//toRTL
...
class UPPCond extends UPPInst {
	...
	RTLInst testCond = new RTLGtz(regCond, ni1, ni2);
	return cond.toRTL(locals, globals, regCond, testCond);
    }//toRTL
...
class UPPProcCall extends UPPInst {
    ...
    RTLInst toRTL (ArrayList<Pair<String,PRegister>> locals,
                   ArrayList<String> globals, RTLInst succ) {
        ArrayList<PRegister> regs = new ArrayList<PRegister>(); // Liste des registres des arguments
        for (UPPExpr e : args)
            regs.add(e.getPRegister(locals)); // Initialisation de la liste
	
        RTLInst proc = new RTLProcCall(callee,regs,succ); // Appel de la procédure
        RTLInst acc = proc;
        for (int i = args.size() - 1; i >= 0; i--)
            acc = args.get(i).toRTL(locals,globals,regs.get(i),acc); // Traduction de tous les arguments en RTL
...
class UPPProc extends UPPDef {
    ...
    RTLDef toRTL (ArrayList<String> globals) {
	ArrayList<PRegister> regArgs = new ArrayList<PRegister>(); // Liste des registres des arguments
        ArrayList<PRegister> regLocals = new ArrayList<PRegister>(); // Liste des registres des variables locales
        ArrayList<Pair<String,PRegister>> regTrans = new ArrayList<Pair<String,PRegister>>(); // Paire de nom de la variable et son registre
