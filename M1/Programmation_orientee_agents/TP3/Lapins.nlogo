...
patches-own [odor maleOdor femaleOdor]
rabbits-own [isMale]
...
to diffuseChemical
  diffuse odor (diffusionRate / 100)
  diffuse maleOdor (diffusionRate / 100)
  diffuse femaleOdor (diffusionRate / 100)

  ask patches [
    set odor (odor - (odor * evaporationRate / 100))
    set maleOdor (maleOdor - (maleOdor * evaporationRate / 100))
    set femaleOdor (femaleOdor - (femaleOdor * evaporationRate / 100))

    ifelse (maleOdor > femaleOdor)
    [set pcolor scale-color blue maleOdor 10 (maxOdor / 1.3)]
    [set pcolor scale-color pink femaleOdor 10 (maxOdor / 1.3)]
  ]
end
@#$#@#$#@
GRAPHICS-WINDOW
