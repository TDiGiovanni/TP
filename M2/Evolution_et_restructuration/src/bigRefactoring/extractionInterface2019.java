...
public class extractionInterface2019
{
	...
	public void createTable(int i) throws SecurityException, ClassNotFoundException
	{
		...
		if (CharacteristicList == null || EntityList == null)
			return;
	...
	public static boolean isCollectionOrMap(String className) throws ClassNotFoundException
	{
		...
		if (Class.forName(className).isAssignableFrom(Collection.class)
				|| Class.forName(className).isAssignableFrom(Map.class))
		{
			return true;
		}
	...
	public static ArrayList<String> finalStaticFieldNamesTypes(String className) throws SecurityException, ClassNotFoundException
	{
		...
		for (Field a : getAllFields(new LinkedList<Field>(), className))
			...
			{
				/*
				 * if(!a.isAccessible()){ a.setAccessible(true); } s+="\t" +
				 * a.getName()+" "+a.getType().getName()+" = "+a.get(objectInstance)+"\n";
				 */
	...
	public static ArrayList<String> methodNameSet(ArrayList<String> classNameList) throws SecurityException, ClassNotFoundException
	{
		...
		for (String c : classNameList)
			for (Method m : Class.forName(c).getMethods())
				if (!liste.contains(m.getName()))
					liste.add(m.getName());
		
		return liste;
	}

	// Extrait toutes les signatures de toutes les classes de la liste passee en parametre
	...
	public static ArrayList<String> signatures(String className) throws SecurityException, ClassNotFoundException
	{
		...
		for (Method m : Class.forName(className).getMethods())
		{
			 ...
			 * exceptionType.getName() + " "; if(m.getExceptionTypes().length != 0) result =
	...
	*/
	...
	/*
	 * private static String concreteClassListNames(String[] classNameList) throws
	 * ClassNotFoundException, UnsupportedEncodingException {
	 * 
	 * String s=""; int nb =0; for (String c : classNameList) if (!
	 * Modifier.isAbstract(Class.forName(c).getModifiers())){ s+=c+"\n";nb++; }
	 * 
	 * System.out.println(nb+" classes concrètes"); return s; }
	 */
	...
	public static void main(String[] args) throws SecurityException, ClassNotFoundException, IOException, IllegalArgumentException, IllegalAccessException
	{
		...
		// de methode
		// le fichier csv peut etre ouvert dans open office (separateur '|') pour
		// manipuler les colonnes
		// et trouver des groupes de signatures communes à des groupes de classes
		// concretes
